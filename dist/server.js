...
app.get("/api/bookings/:id", (req, res) => {
    db_1.default.query("SELECT * FROM bookings WHERE id=?", [
        req.params.id
    ], (err, result) => {
        if (err) {
            return res
...
app.post("/api/bookings", (req, res) => {
    const { room_name, topic, booker, department, meeting_date, start_time, end_time, participants } = req.body;
    const checkSQL = `
    ...
    db_1.default.query(checkSQL, [
        ...
        if (result.length > 0) {
            return res.json({
                success: false,
                ai_message: "🤖 AI : ห้องประชุมนี้ถูกจองแล้ว"
            });
        }
        const status = "จองแล้ว";
        const ai_message = "🤖 AI : จองห้องประชุมเรียบร้อย";
        const insertSQL = `
...
app.get("/api/check-room", (req, res) => {
    ...
    db_1.default.query(sql, [
        ...
        if (result.length > 0) {
            return res.json({
                available: false,
                ai_message: "🤖 AI : ห้องประชุมไม่ว่าง"
            });
        }
        res.json({
            available: true,
            ai_message: "🤖 AI : ห้องพร้อมใช้งาน"
        });
    });
});
// =====================
...
app.put("/api/bookings/:id", (req, res) => {
    ...
    const { room_name, topic, booker, department, meeting_date, start_time, end_time, participants, status } = req.body;
    let ai_message = "🤖 AI : บันทึกข้อมูลเรียบร้อย";
    switch (status) {
        case "จองแล้ว":
            ai_message =
...
app.delete("/api/bookings/:id", (req, res) => {
    db_1.default.query("DELETE FROM bookings WHERE id=?", [
        req.params.id
    ], (err) => {
        if (err) {
            return res
